...
namespace ConsensusChessShared.Service
{
	...
	{
        public ServiceConfig(string tag, string admin, Visibility publicVisibility, IEnumerable<string> ignorables, bool streamEnabled, string additionalTags)
        {
            GameTag = tag;
            AdminContact = admin;
        ...
        public static ServiceConfig FromEnv(IDictionary env)
        {
            ...
            var additionalTags = environment["POST_ADDITIONAL_TAGS"];
            ...
            return new ServiceConfig(tag, admin, publicVisibility, ignorables, streamEnabled, additionalTags);
        }
    }
}
